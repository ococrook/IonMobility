...
To load a Spectra object from a file, first provide the file path for example
using `fl <- myfilepath`. Here is an example from within the package itself:

```{r,}
fpath <- system.file("extdata", "phosb_test",
                     package = "IonMobility", mustWork = TRUE)
```

Now we can read it in as a Spectra object.
```{r, cache = TRUE, warning=FALSE}
phosb_test <- readRDS("phosb_test.rds")
```
